...
namespace Core.Business.ViewModels.Validators.Properties
{
    public class AccountValidator : BaseValidator<User>
    {
        ...
        private bool SignInViewModelProcess(PropertyValidatorContext context)
        {
            ...
            var user = _unitOfWork.RepositoryCRUD<User>().GetSingle(
        ...
        {
            return !_unitOfWork.RepositoryR<User>().Any(x => x.UserName.EqualsIgnoreCase(userName));
        }

        public bool UniqueCode(string code)
        {
            return !_unitOfWork.RepositoryR<User>().Any(x => x.Code.EqualsIgnoreCase(code));
        }
        ...
        public bool UniqueEmail(string Email)
        {
            ...
            {
                return !_unitOfWork.RepositoryR<User>().Any(x => x.Email.Equals(Email));
            }
        ...
        public bool UniqueEmail(UpdateAccountViewModel model)
        {
            if (!string.IsNullOrEmpty(model.Email))
            {
                ...
                if (model.Id == 0)
                {
                    return !_unitOfWork.RepositoryR<User>().Any(x => x.Email.Equals(model.Email));
                }
                else
                {
                    return !_unitOfWork.RepositoryR<User>().Any(x => x.Email.Equals(model.Email) && x.Id != model.Id);
                }
            }
            return true;
        }
        ...
        {
            return !_unitOfWork.RepositoryR<User>().Any(x => x.PhoneNumber.Equals(Phonenumber));
        }
        ...
        public bool UniquePhone(UpdateAccountViewModel model)
        {
            if (model.Id == 0)
            {
                return !_unitOfWork.RepositoryR<User>().Any(x => x.PhoneNumber.Equals(model.PhoneNumber));
            }
            else
            {
                return !_unitOfWork.RepositoryR<User>().Any(x => x.PhoneNumber.Equals(model.PhoneNumber) && x.Id != model.Id);
            }
        }
        ...
        public bool UniqueCode(UpdateAccountViewModel model)
        {
            if (model.Id == 0)
            {
                return !_unitOfWork.RepositoryR<User>().Any(x => x.Code.EqualsIgnoreCase(model.Code));
            }
            else
            {
                return !_unitOfWork.RepositoryR<User>().Any(x => x.Code.EqualsIgnoreCase(model.Code) && x.Id != model.Id);
            }
        }

        public bool PassWordValid(ChangePassWordViewModel model)
        {
            return _unitOfWork.RepositoryCRUD<User>().Any(
                x => x.Id == model.UserId &&
